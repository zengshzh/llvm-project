...
bool llvm::toObfuscate(bool flag, Function *f,
  ...
  if (getFunctionAnnotation(f).find(attr) != std::string::npos) { // 是否开启XXX
    outs() << "[obf] function: " << f->getName().str() << " OPEN " << attr << " by annotate\n";
    return true;
  }
...
void llvm::fixStack(Function &F) {
  ...
  CastInst *AllocaInsertionPoint = new BitCastInst(
      ...
      Type::getInt32Ty(F.getContext()), "fix_stack_point", I->getIterator());
  ...
  for (Instruction *I : WorkList)
    DemoteRegToStack(*I, false, AllocaInsertionPoint ? std::optional<BasicBlock::iterator>(AllocaInsertionPoint->getIterator()) : std::nullopt);
  ...
  for (Instruction *I : WorkList)
    DemotePHIToStack(cast<PHINode>(I), AllocaInsertionPoint ? std::optional<BasicBlock::iterator>(AllocaInsertionPoint->getIterator()) : std::nullopt);
}
